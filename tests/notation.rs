...
fn both() {
    ...
    assert_eq!(game.move_to_an(&m.as_ref().unwrap(), true, false), "Ncxe4");
    ...
    assert_eq!(game.move_to_an(&game.an_to_move("N3e4", Color::White).unwrap(), true, false), "Ncxe4");
    assert_eq!(m, game.an_to_move("Nc3e4", Color::White));
    assert_eq!(game.move_to_an(&game.an_to_move("Nc3e4", Color::White).unwrap(), true, false), "Ncxe4");
    game.move_pieces(&m.unwrap());
    
    // Fast forwards some more.
    m = game.an_to_move("Qf3", Color::White);
    assert_eq!(game.move_to_an(&m.as_ref().unwrap(), true, false), "Qf3");
    game.move_pieces(&m.unwrap());
    m = game.an_to_move("Be2", Color::White);
    assert_eq!(game.move_to_an(&m.as_ref().unwrap(), true, false), "Be2");
    game.move_pieces(&m.unwrap());
    m = game.an_to_move("b3", Color::White);
    assert_eq!(game.move_to_an(&m.as_ref().unwrap(), true, false), "b3");
    game.move_pieces(&m.unwrap());
    m = game.an_to_move("Bb2", Color::White);
    assert_eq!(game.move_to_an(&m.as_ref().unwrap(), true, false), "Bb2");
    game.move_pieces(&m.unwrap());
    
    // Kingside castling.
    m = game.an_to_move("0-0", Color::White);
    assert_eq!(game.move_to_an(&m.as_ref().unwrap(), true, false), "0-0");
    assert_eq!(m, Some(vec![((4, 0), (5, 0)), ((5, 0), (6, 0)), ((7, 0), (5, 0))]));
    
    // Queenside castling.
    m = game.an_to_move("0-0-0", Color::White);
    assert_eq!(game.move_to_an(&m.as_ref().unwrap(), true, false), "0-0-0");
